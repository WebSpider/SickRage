...
%>
    <div class="show" id="show${curShow.indexerid}" data-name="${curShow.name}" data-date="${data_date}" data-network="${curShow.network}" data-progress="${progressbar_percent}">
        <div class="show-image">
            <a href="${sbRoot}/home/displayShow?show=${curShow.indexerid}"><img alt="" class="show-image" src="${sbRoot}/showPoster/?show=${curShow.indexerid}&amp;which=poster_thumb&amp;v=${sickbeard.CUR_COMMIT_HASH[:6]}" /></a>
        </div>
...
% if sickbeard.TRAKT_USE_ROLLING_DOWNLOAD and sickbeard.USE_TRAKT:
            <img src="${sbRoot}/images/${('no16.png?v=${sickbeard.CUR_COMMIT_HASH[:6]}", alt="No"', 'yes16.png?v=${sickbeard.CUR_COMMIT_HASH[:6]}", alt="Yes"')[int(curShow.paused) == 0]} width="16" height="16" />
% else:
